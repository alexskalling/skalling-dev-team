...
Use `teamdb-plan.sh` to create the plan + tasks atomically in the DB:
```bash
# First, compose your tasks.md content in memory, then:
bash ~/.config/opencode/scripts/teamdb-plan.sh \
  "$PROJECT" \
  "{change-name}" \
  "{Change Title}" \
  /dev/stdin \
...
# Input stdin: task list in - [ ] Title format
```

Or for existing plans, use `teamdb-amend.sh --add-task`:
```bash
bash ~/.config/opencode/scripts/teamdb-amend.sh "{change-name}" \
  --add-task="{task title}" \
...
Use this template to structure the tasks content before writing to DB:
...
**Then write to DB** using `teamdb-plan.sh` (for new plans) or `teamdb-amend.sh --add-task` (for existing plans), incorporating this composed content.
...
- **openspec**: After the DB write in Step 3, export to filesystem via `teamdb-export-md.sh`. The file is GENERATED, not source.
...
**Change**: {change-name}
**Location**: DB (source of truth) | `openspec/changes/{change-name}/tasks.md` (openspec/hybrid export) | Engram `sdd/{change-name}/tasks` (engram)
...
- **DB is source of truth** — never write `.md` files as if they were source. Use `teamdb-plan.sh` or `teamdb-amend.sh` for DB persistence.
