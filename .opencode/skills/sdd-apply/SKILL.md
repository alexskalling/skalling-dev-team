...
### Step 5: Mark Tasks Complete in DB

**CRITICAL**: Never write `[x]` marks directly to `.md` files. The `.md` is an export, not the source.

Update task status in TeamDB:

```bash
# Mark a task as done:
bash ~/.config/opencode/scripts/teamdb-amend.sh "{plan-slug}" \
  --modify-task="task-slug" \
  --new-status="done" \
  "$PROJECT"
...
python3 ~/.config/opencode/scripts/teamdb_exec.py \
  ...
  --mode write \
  --sql "UPDATE tasks SET status = 'done', updated_at = datetime('now') WHERE plan_id = (SELECT id FROM plans WHERE slug = ?) AND slug = ?" \
  --params '["{change-name}", "task-slug"]'
```

After DB update, export to filesystem if user visibility is needed:
```bash
...
- type: `architecture`
- Also update task status in DB via `teamdb_exec.py --mode write` (openspec/hybrid) or `mem_update` (engram).
...
- **DB is source of truth for task status** — never write `[x]` marks directly to `.md` files. Use `teamdb_exec.py --mode write` or `teamdb-amend.sh` for DB updates.
...
- In `openspec` mode, mark tasks complete in DB AS you go, not at the end
- Before returning, re-read the persisted tasks artifact (DB for openspec, Engram for engram) and ensure completed tasks have status `done`; internal todos are not completion evidence
