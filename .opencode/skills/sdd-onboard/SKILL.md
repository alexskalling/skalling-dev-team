...
**CRITICAL**: DB is source of truth. Write proposal to DB first via `teamdb_exec.py --mode write`, then export to filesystem for user visibility.

Write proposal to DB following `sdd-propose` format. After creating it:
...
Show the user the proposal (from exported `.md` if needed) and let them review it. Ask if they want to adjust anything before continuing.
...
**CRITICAL**: DB is source of truth. Write specs to DB first, then export.
...
**CRITICAL**: DB is source of truth. Write design to DB first, then export.

Write design following `sdd-design` format. Highlight the key decisions:
...
**CRITICAL**: DB is source of truth. Write tasks to DB first via `teamdb-plan.sh`, then export.

Write tasks following `sdd-tasks` format. Explain the structure:
...
**CRITICAL**: DB is source of truth for task status. Mark tasks complete in DB via `teamdb_exec.py --mode write`, not by editing tasks.md.
...
**Change**: {change-name}
**Artifacts created** (DB is source of truth):
- proposals table — the WHY
- specs table — the WHAT
- plans table (design_md) — the HOW
- tasks table — the STEPS
- Exported to .md files in `.opencode/changes/{change-name}/` for visibility
...
- Check TeamDB via `teamdb-status.sh` — that's your growing source of truth
- Questions? The orchestrator is always available
```

## Rules

- **DB is source of truth** — all SDD artifacts (proposals, specs, design, tasks) go to TeamDB first, then export to `.md` for visibility.
