...
**CRITICAL**: DB is the source of truth. When reading artifacts, use DB queries (`teamdb_exec_query`) for openspec/hybrid mode, not filesystem reads.
...
2. **Read from DB** (`teamdb_exec_query`) for openspec/hybrid modes. Example queries:
   ```bash
   # Get task status for a plan:
   teamdb_exec_query "$DB" "SELECT slug, title, status FROM tasks WHERE plan_id = (SELECT id FROM plans WHERE slug = ?)" "{change-slug}"
   
   # Get design content:
   teamdb_exec_query "$DB" "SELECT design_md FROM plans WHERE slug = ?" "{change-slug}"
   ```
3. Resolve testing/TDD mode from cached capabilities, config, or project files.
4. Count completed and incomplete tasks from DB. Any `pending` task blocks full verification; focused checks remain an apply work-unit responsibility.
