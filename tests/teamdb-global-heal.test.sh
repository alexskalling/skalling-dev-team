...
if [ "$VER" = "0.7.2" ]; then
  assert_pass "install heals: schema_meta.version al día (0.7.2)"
else
  assert_fail "install heals: schema_meta.version al día (0.7.2)" "ver=$VER"
fi
...
export HOME="$HOME_BAK"
if [ "$RC_HEAL2" = "0" ] && [ "$VER2" = "0.7.2" ] && [ "$PREFS2" = "2" ]; then
...
if [ -n "$HAS_TABLE" ] && [ "$HAS_COL2" = "1" ] && [ "$VER3" = "0.7.2" ]; then
  assert_pass "DB global nueva usa schema actual (audit_log + actor_source + 0.7.2)"
else
