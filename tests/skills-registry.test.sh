...
# ── C) teamdb-init de proyecto llega a 0.7.8 con skills_registry ──
PROJ="$TMP/proj"
mkdir -p "$PROJ"
...
if [ "$P_VER" = "0.9.0" ] && [ "$P_SKILLS" = "1" ]; then
  assert_pass "teamdb-init proyecto: 0.9.0 + skills_registry"
else
  assert_fail "teamdb-init proyecto: 0.9.0 + skills_registry" "ver=$P_VER skills=$P_SKILLS"
fi
