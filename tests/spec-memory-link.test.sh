...
test_version_bump_y_changelog() {
    echo ""
    echo "── Test 4.4 / 4.5: bump 0.6.0 y entrada CHANGELOG ──"

    local version_actual
    version_actual="$(grep -E '^__version__' "$VERSION_FILE" | sed -E 's/.*"([^"]+)".*/\1/')"
    if [[ "$version_actual" == "0.6.0" ]]; then
        pass "VERSION = 0.6.0"
    else
        fail "VERSION != 0.6.0 (obtenido: $version_actual)"
    fi

    if grep -q 'SKALLING_VERSION="0.6.0"' "$DOCTOR"; then
        pass "doctor SKALLING_VERSION = 0.6.0"
    else
        fail "doctor SKALLING_VERSION != 0.6.0"
    fi

    if grep -q 'SKALLING_VERSION="0.6.0"' "$ROOT/install-global.sh"; then
        pass "install-global.sh SKALLING_VERSION = 0.6.0"
    else
        fail "install-global.sh SKALLING_VERSION != 0.6.0"
    fi
