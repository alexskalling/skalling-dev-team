...
EXPECTED_VERSION=$(grep '__version__' "$ROOT/VERSION" | sed 's/.*"\([^"]*\)".*/\1/')
if [ "$VER_PROJ" = "$EXPECTED_VERSION" ] && [ "$HAS_ACTOR" = "1" ] && [ -n "$HAS_HISTORY" ]; then
  assert_pass "proyecto instalado: version $EXPECTED_VERSION + actor_source + migraciones 003"
else
  assert_fail "proyecto instalado: version $EXPECTED_VERSION + actor_source + migraciones 003" \
...
if [ "$G_VER" = "$EXPECTED_VERSION" ] && [ "$G_SKILLS" -gt 0 ]; then
  assert_pass "install: team.db global $EXPECTED_VERSION + skills_active poblado ($G_SKILLS skills)"
else
