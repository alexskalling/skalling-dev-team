...
if [ "$VER_PROJ" = "0.7.6" ] && [ "$HAS_ACTOR" = "1" ] && [ -n "$HAS_HISTORY" ]; then
  assert_pass "proyecto instalado: version 0.7.6 + actor_source + migraciones 003"
else
  assert_fail "proyecto instalado: version 0.7.6 + actor_source + migraciones 003" \
...
if [ "$G_VER" = "0.7.6" ] && [ "$G_SKILLS" -gt 0 ]; then
  assert_pass "install: team.db global 0.7.6 + skills_active poblado ($G_SKILLS skills)"
else
  assert_fail "install: team.db global 0.7.6 + skills_active poblado" "ver=$G_VER skills=$G_SKILLS"
fi
