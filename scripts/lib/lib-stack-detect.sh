...
# Obtiene la lista de skills recomendadas para el framework detectado.
# Output: una skill por línea.
skalling_recommended_skills_for_stack() {
    local yaml_file="$1"
    local stack_key="$2"
    ...
    # Buscar la sección top-level que matchea el stack_key
    awk -v key="$stack_key" '
        BEGIN { in_section=0 }
        /^[a-z_-]+:$/ {
            current=tolower($1)
        ...
        in_section && /^custom:/ { exit }
        in_section && /^[a-z]/ && !/^[a-z_-]+:$/ {
            # Salió de la sección (otro top-level)
            in_section = 0
        }
        in_section && /^  - name:/ {
            line=$0
...
# Lista de skills marcadas como "always: true" en la sección core.
skalling_core_skills() {
    local yaml_file="$1"

    [[ ! -f "$yaml_file" ]] && return 1

    # Solo los items con name: bajo "core:" (los stack-specific están en otras secciones)
    awk '
        /^core:/ { in_core=1; next }
        in_core && /^  - name:/ {
            line=$0
        ...
        # Cualquier otra sección top-level termina core
        in_core && /^[a-z]/ && !/^core:/ && !/^[[:space:]]/ { in_core=0 }
    ' "$yaml_file"
}
