...
for f in "${md_files[@]}"; do
  ...
  # Verificar si ya existe (parameter binding: real SQL escaping, no manual)
  # Antes: sqlite3 "$DB" "...WHERE slug='$slug_escaped'" con sed ' -> '' que
  # protege contra ' pero NO contra ; NUL, comentarios --, etc. Defense in
  # depth: usar teamdb_exec_value con bind params (Python sqlite3 → R10).
  existing="$(teamdb_exec_value "$DB" "SELECT COUNT(*) FROM proposals WHERE slug=? LIMIT 1" "$slug_imported" 2>/dev/null || echo "0")"
  if [ "$existing" != "0" ]; then
